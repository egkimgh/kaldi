...
if [ $# -lt 1 ] || [ $# -gt 3 ]; then
   ...
   echo "e.g.: $0 data/train exp/make_spectrogram/train spect"
...
# make $spectdir an absolute pathname.
spectdir=`perl -e '($dir,$pwd)= @ARGV; if($dir!~m:^/:) { $dir = "$pwd/$dir"; } print $dir; ' $spectdir ${PWD}`

# use "name" as part of name of the archive.
name=`basename $data`

mkdir -p $spectdir || exit 1;
...
for n in $(seq $nj); do
  # the next command does nothing unless $spectdir/storage/ exists, see
  # utils/create_data_link.pl for more info.
  utils/create_data_link.pl $spectdir/raw_spect_$name.$n.ark
done
...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_spect_${name}.JOB.log \
    ...
    compute-spectrogram-feats $vtln_opts --verbose=2 --config=$spect_config ark:- ark:- \| \
    copy-feats --compress=$compress $write_num_frames_opt ark:- \
     ark,scp:$spectdir/raw_spect_$name.JOB.ark,$spectdir/raw_spect_$name.JOB.scp \
     || exit 1;
  ...
  $cmd JOB=1:$nj $logdir/make_spect_${name}.JOB.log \
    compute-spectrogram-feats $vtln_opts --verbose=2 --config=$spect_config scp,p:$logdir/wav.JOB.scp ark:- \| \
    copy-feats --compress=$compress $write_num_frames_opt ark:- \
     ark,scp:$spectdir/raw_spect_$name.JOB.ark,$spectdir/raw_spect_$name.JOB.scp \
     || exit 1;
...
for n in $(seq $nj); do
  cat $spectdir/raw_spect_$name.$n.scp || exit 1;
done > $data/feats.scp
