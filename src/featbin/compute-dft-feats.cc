...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Create dft feature files.\n"
        "Usage:  compute-dft-feats [options...] <wav-rspecifier> <feats-wspecifier>\n";
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      try {
        dft.ComputeFeatures(waveform, wave_data.SampFreq(), 1.0, &features);
      } catch (...) {
