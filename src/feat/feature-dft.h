...
#ifndef KALDI_FEAT_FEATURE_DFT_H_
#define KALDI_FEAT_FEATURE_DFT_H_
...
/// DftOptions contains basic options for computing dft
/// features.
struct DftOptions {
  ...
  void Register(OptionsItf *opts) {
    ...
    opts->Register("energy-floor", &energy_floor,
                   ...
                   "floor on the individual dft elements is fixed at "
...
/// Class for computing dft features.
class DftComputer {
 public:
  typedef DftOptions Options;
  explicit DftComputer(const DftOptions &opts);
  DftComputer(const DftComputer &other);
  ...
  /**
     Function that computes one frame of dft features from
     one frame of signal.
 ...
 private:
  ...
  // Disallow assignment.
  DftComputer &operator=(const DftComputer &other);
};

typedef OfflineFeatureTpl<DftComputer> DFT;
...
#endif  // KALDI_FEAT_FEATURE_DFT_H_
